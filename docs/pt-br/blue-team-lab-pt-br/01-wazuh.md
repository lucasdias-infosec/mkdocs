...
Optou-se pela instalação em modo single-node (all-in-one), no qual:
...
operam na mesma máquina virtual.
...
O status confirmado foi Active, indicando que o monitoramento local encontra-se operacional.
